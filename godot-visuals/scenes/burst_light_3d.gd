...
@export var col_white_boost: float = 1.0
@export var peak_energy: float = 10.0
@export var incr_time: float = 10.32
@export var decr_time: float = 1.32
@export var range_start: float = 3.0
@export var range_end: float = 18.0
...
func spawn_burst_light(
	...
	cast_shadows: bool = false, #should be true for hero lights
	...
	t2.tween_property(L, "omni_range", range_end, incr_time)
	t2.finished.connect(func ():
		t2.tween_property(L, "omni_range", 0.0, decr_time)
	)
	#t.parallel().tween_property(L, "omni_range", range_end, incr_time)
